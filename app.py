...
nxtYear = date.today() + relativedelta(years=1)
...
app = dash.Dash(__name__, external_stylesheets=[bootstrap.themes.FLATLY],
                meta_tags=[{'name': 'viewport',
                            'content': 'width=device-width, initial-scale=1.0'}])
server = app.server
app.title = 'Travel Buddy- A Flight Price Prediction System'
app._favicon = "plane.png"

app.layout = bootstrap.Container([
    bootstrap.NavbarSimple(
        children=[
            bootstrap.NavItem(
                html.A([bootstrap.CardImg(src='assets/travel.png', style={'width': '70px', 'height': '60px'}
                                          , className='mx-5 my-1')],
                       href='#')),
            bootstrap.NavItem(
                bootstrap.NavLink(html.A("Predict", href="#", className="me-1 text-decoration-none fs-5"))),
            bootstrap.NavItem(
                bootstrap.NavLink(html.A("Model", href="#model-report", className="me-1 text-decoration-none fs-5"))),
            bootstrap.NavItem(
                bootstrap.NavLink(html.A("Analysis", href="#eda", className="me-1 text-decoration-none fs-5"))),

        ], fixed='top',
        brand="Travel Buddy Dashboard",
        brand_href="#",
        color="primary",
    ...
    html.Br(),
    html.Br(),
    bootstrap.Row([html.H1("Travel Buddy- A Flight Price Prediction System", style={'text-align': 'center'})]),
    html.Br(),
    html.Br(),

    bootstrap.Row([
        functions.inputs('Airline', 'airline-dpn', 'Select Airline', airlines, offset=1),
        functions.inputs('Source', 'source-dpn', 'Select Source', source),
        functions.inputs('Destination', 'dest-dpn', 'Select Destination', destination),
        functions.inputs('Stops', 'stops-dpn', 'Select Stops', [0, 1, 2, 3, 4], offset=0),
    ], justify='center'),
    html.Br(),
    html.Br(),
    html.Br(),
    ...
    bootstrap.Row([
        bootstrap.Col([html.Header('Date of Journey'),
                       dcc.DatePickerSingle(
                           id='doj',
                           placeholder='dd-mm-yyyy',
                           min_date_allowed=datetime.date.today(),
                           max_date_allowed=nxtYear,
        ...
        bootstrap.Col([html.Header('Departure Time(HH:MM)'),
                       functions.selectTime('dhour_dpdn', 'dmin_dpdn')
                       ], xl={'size': 2, 'offset': 1}, lg={'size': 2, 'offset': 1}, md={'size': 2, 'offset': 1},
                      sm={'size': 6, 'offset': 1}, xs={'size': 6, 'offset': 1}, style={'text-align': 'center'}),

        bootstrap.Col([html.Header('Travel Time (HH:MM)'),
                       functions.selectTime('thour_dpdn', 'tmin_dpdn')
                       ], xl={'size': 2, 'offset': 1}, lg={'size': 2, 'offset': 1}, md={'size': 2, 'offset': 1},
                      sm={'size': 6, 'offset': 1}, xs={'size': 6, 'offset': 1}, style={'text-align': 'center'}),

        functions.inputs('Class', 'class_dpdn', 'Select Class', f_class),

    ], justify='center'),
    bootstrap.Row([
        bootstrap.Col([
            bootstrap.Button('Predict', id='button', n_clicks=0),
    ...
    bootstrap.Row([
        bootstrap.Col([
            bootstrap.Label("Model: Random Forest Regressor", className='fs-5'),
            dcc.RadioItems(['Accuracy Score', 'Feature Importance', 'Histogram', 'KDE', 'Scatter-plot'],
                           'Accuracy Score',
                           id='result-radio'),
            html.Br(),
    ...
    bootstrap.Row([
        bootstrap.Col([
            bootstrap.Row([
                html.A('Developed by: Fatima Nevrekar', className='me-1 text-decoration-none fs-5',
                       href='#')
            ])], style={'text-align': 'center'})
...
def update_output(n_clicks, v, v2, v3, v4, v5, v6, v7, v8, v9, v10):
    if n_clicks == 0:
        ...
        stops = [v4]
        month_day_week = functions.dateFormat(v5)
        dep_hour_min = [v6, v7]
        duration_hour_min = [v8, v9]
        class_f = [functions.fClass(v10)]
        arrival_hour_min = functions.ArivalTime(v6, v7, v8, v9)
...
def update_bar(x):
    ...
    fig = pex.bar(data_frame=temp, x='index', y=x, color='index', template='plotly_dark',
...
def update_histogram(x, y):
    ...
    fig = pex.histogram(df, x=x, marginal="box", color=color, template='plotly_dark', labels={'count': 'No of Records'})
...
def update_bar(x):
    fig = pex.box(df, x=x, y="Price", color=x, template='plotly_dark')
...
def update_bar(y, color, size):
    ...
    fig = pex.scatter(df, x='Price', y=y, color=color, size=size, template='plotly_dark', labels={y: y_label})
...
def update_bar(x, y):
    ...
    fig = pex.imshow(d, text_auto=True, origin='lower',
...
def update_bar(x):
    ...
    fig = pgo.Figure(pgo.Scattermapbox(
        mode="markers",
        lon=longitude,
        lat=latitude,
        text=locations,
...
def update_bar(x):
    y_pred = model.predict(test_dataframe.iloc[:, :-1].values)
    actual_val = test_dataframe.iloc[:, -1]
    d = pd.DataFrame()
    ...
    elif x == 'Features Importance':
        fi = pd.DataFrame(model.feature_importances_,
                          index=test_dataframe.iloc[:, :-1].columns).reset_index().sort_values(
            by=0).rename(
            columns={'index': 'feature', 0: 'value'})

        fig = pex.bar(fi, x='feature', y='value', color='feature', template='plotly_dark')
        fig.update_layout(title_text='Feature Importance as per Random Forest Regressor', title_x=0.5)
        return fig
    elif x == 'Histogram':

        fig = pex.histogram(x=hist_data2, marginal="rug", labels={'x': 'Error Price'})
        fig.update_layout(title_text='Actual - Predicted Error', title_x=0.5)

        return fig
    elif x == 'KDE':
        pi.templates.default = "plotly_dark"
        fig = pff.create_distplot(hist_data, group_labels, show_hist=False, colors=colors)
        fig.update_layout(title_text='Actual vs Predicted Result on Test data', title_x=0.5)
        return fig
    elif x == 'Scatter-plot':
        fig2 = pex.scatter(d, x='Actual', y='Predicted')
